<template>
    ...
    <div :class="$style.toolContainer">
        <template v-if="selectedTool">
            ...
            <div :class="$style.toolGitRepositoryContentWrapper">
                <div :class="$style.toolGitRepositoryContent">
                    ...
                    <section :class="$style.toolGitRepositoryUser">
                        ...
                        <template v-if="gitRepositoryUser">
                            <a v-if="selectedTool.git_vendor === 'github'" :href='gitRepositoryUser.html_url'>
                                {{ gitRepositoryUser.login }}
                            </a>
                            <a v-else-if="selectedTool.git_vendor === 'gitlab'" :href='gitRepositoryUser.web_url'>
                                {{ gitRepositoryUser.username }}
                            </a>
                        </template>
                        <p v-else>N/A</p>
                    </section>
                    <section :class="$style.toolGitRepositoryBranches">
                        <div>
                        ...
                        <ul v-if="gitRepositoryBranches && gitRepositoryBranches.length > 0">
                            <li v-for="branch in gitRepositoryBranches">
                                <a href='#'>- {{ branch.name }}</a>
                            </li>
                        </ul>
                    </section>
                    <section :class="$style.toolGitRepositoryContributors">
                        <div>
                ...
                <div :class="$style.toolGitRepositoryContent">
                    <section :class="$style.toolGitRepositoryReleases">
                        ...
                        <template v-if="gitRepositoryReleases && gitRepositoryReleases.length > 0">
                            <ul v-if="selectedTool.git_vendor === 'github'">
                                <li v-for="(release, index) in gitRepositoryReleases">
                                    <a v-if="index < 15" :href='release.zipball_url'>{{ release.name }}</a>
                                </li>
                            </ul>
                            <ul v-else-if="selectedTool.git_vendor === 'gitlab'">
                                <li v-for="(release, index) in gitRepositoryReleases">
                                    <a v-if="index < 15" :href='release._links.self'>{{ release.name }}</a>
                                </li>
                            </ul>
                        </template>
                    ...
                    <section :class="$style.toolGitRepositoryTags">
                        ...
                        <ul v-if="gitRepositoryTags && gitRepositoryTags.length > 0">
                            <li v-for="(tag, index) in gitRepositoryTags">
                                <a  v-if="index < 15" :href='tag.zipball_url'>- {{ tag.name }}</a>
                            </li>
                        </ul>
...
const gitRepositoryBranches = ref()
const gitRepositoryContributors = ref()
...
onMounted(async () => {
    ...
    if (selectedTool.value && selectedTool.value.git_repository_url && selectedTool.value.git_vendor) {
        ...
        if (repository.value) {
            ...
            gitRepositoryBranches.value = await repository.value.getBranches()
            console.log('BRANCHES: ', gitRepositoryBranches.value)

            gitRepositoryContributors.value = await repository.value.getContributors()
            console.log('CONTRIBUTORS: ', gitRepositoryContributors.value)
        }
    }
})
</script>